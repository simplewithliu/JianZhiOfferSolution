...
你会得到一个双链表，其中包含的节点有一个 下一个指针、一个 前一个指针和一个 额外的子指针。
这个子指针可能指向一个单独的双向链表，也包含这些特殊的节点。这些子列表可以有一个或多个自己的子列表，以此类推，以生成如下面的示例所示的多层数据结构 。
...
private:
	Node *dfs(Node *node) {
		...
		while (node) {
			if (node->child) {
				Node *tmp = node->next;
				Node *cLast = dfs(node->child);
				node->next = node->child;
				node->child->prev = node;
				node->child = nullptr;
				cLast->next = tmp;
				if (tmp) tmp->prev = cLast;
				last = cLast;
				node = tmp;
			} else {
				last = node;
